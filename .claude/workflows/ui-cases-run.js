export const meta = {
  ...
  whenToUse: 'The desktop (daily, page:"all") and mobile (hourly, page:"auto") UI-testing routines. Args {surface, page, date, maxCasesPerPage?, targetUrl?}. Contract: Case Study/UI Cases/Claude Audit/RUNBOOK.md + GUARDS.md.',
  phases: [
...
const UIC = 'Case Study/UI Cases/Claude Audit'
...
WINDOWS/ENCODING: prefix every python invocation with PYTHONUTF8=1 (e.g. \`PYTHONUTF8=1 python "${UIC}/tools/status.py" ...\`) and never print non-ASCII (→, ✔, em-dashes) to stdout — the Windows console is cp1252 and will crash on it. Ledger/report FILES are UTF-8 (the tools handle that); only stdout is the hazard.

BROWSER — this surface uses **${surface === 'mobile' ? 'chrome-devtools MCP' : 'Playwright MCP'}** (load via ToolSearch). The desktop and mobile routines deliberately use DIFFERENT browser tools so they can run SIMULTANEOUSLY without contending for one browser. ${surface === 'mobile' ? "Open the app with chrome-devtools 'new_page', then 'emulate' 275x596 CSS @ DPR 5.24, isMobile+hasTouch; drive with real taps/swipes and the bottom nav. (Its own separate Chrome instance.)" : "Use Playwright (browser_navigate/click/type/…); it owns download capture for the export/import cases. (Its own separate browser instance.)"} Do NOT use Claude-in-Chrome / the user's real Chrome window — this routine drives its own headless browser instance. Isolation is NOT required: the deployed mock is per-browser-profile and the user has approved recon mutations STICKING, so a persistent profile is fine; each surface keeps its own independent mock state.

GATES, in order:
 1. Reach + login: open ${url}, log in through the real login form as admin/admin (typing into the form — never inject a token/localStorage). Record whether the app loaded and login succeeded.
 2. Backend: after login, if admin/admin was REJECTED the deployed build is on the real Supabase backend, not the mock -> run READ-ONLY cases only, mark every mutation/import case BLOCKED_ENVIRONMENT, set backend:"real-backend". Otherwise backend:"mock" and mutations are allowed (they may stick — that is approved).
...
WORK: run '${p}' cases in CASES.md order, RESUMING from the ledger. Determine the resume point by reading '${isoDir(p)}/ledger.jsonl' (a case with step lines but no 'case' line is in-flight -> continue at its first missing step; else the first case with no 'case' line; when none remain, do a REGRESSION lap on cases whose last build != buildStart). Budget: up to ${maxCases} case-slots (any authored case — workflow/metric-proof/chain/round-trip and the adversarial/cross-cutting types failure/theme/boundary/permission-boundary/a11y/nav-state/session/responsive/network/copy = 2; atomic = 1). Priority: in-flight -> proofs/workflows/chains/round-trips/adversarial-edge/cross-cutting -> atomics. For any negative/adversarial case the evidence MUST capture the ADVERSE condition actually occurring (the 403/conflict/error text, the applied theme, the overflow render, the empty state, the second submit) — a condition that never fired is BLOCKED_BY_UI_PREREQUISITE, never PASS (GUARDS "Evidence bars"). Theme cases switch theme via the UI picker ONLY (never inject the localStorage keys); a silent cross-scope write in a permission-boundary case is a CONFIRMED_ISSUE. a11y cases run an axe-core scan (or a scripted DOM check) + a keyboard/focus-trap/Escape pass + 200% text-zoom — axe / Navigation-Timing / DOM reads are MEASUREMENT of the rendered page, NOT state injection (allowed, U12). network cases record time-to-interactive as a ledger note (tti_ms=<n> at build=<hash>) even if throttling is unavailable; if throttle or session-expiry cannot be forced, mark that sub-assertion BLOCKED_BY_UI_PREREQUISITE / BLOCKED_ENVIRONMENT.

FOR EACH CASE: perform the real UI steps (clicks/drags/hovers/dropdowns/scrolls/${surface === 'mobile' ? 'taps/swipes' : 'keyboard'}). APPEND (>>, never overwrite) one JSON 'step' line per step to '${isoDir(p)}/ledger.jsonl' AS YOU GO, each quoting the OBSERVED UI TEXT VERBATIM (U4) and pointing at an evidence file you saved under '${isoDir(p)}/evidence/<RUN_TS>/'. Close each case with a 'case' line {kind:"case",ts,run:<RUN_TS>,surface:"${surface}",case,status,build:buildStart,severity,finding,evidence,note}. ACCOUNTS: default session is admin/admin. If a case's 'Roles:' names a non-admin (e.g. member1, branch1), log in as that user (ALL seeded users' password is 'admin') for that case, then RESTORE admin before the next case unless it names its own account; record the account in the case-line note. Mutations are UI-only (never inject state) and MAY STICK — the user approved that. Read baselines from the CURRENT live state immediately before mutating (never assume pristine seed — prior runs' changes persist). Export cases: capture the download, PARSE it, quote parsed vs on-screen values. Import cases: run 'PYTHONUTF8=1 python "${UIC}/tools/make-import-fixture.py" --out "${isoDir(p)}/evidence/<RUN_TS>" --ts <RUN_TS>', upload it, assert the accepted/rejected counts against the fixture oracle, then DELETE the UICASE rows via the UI (hygiene — keeps test contacts from piling up; the tag makes strays findable). Reverting other mutations is OPTIONAL.

FINDINGS: dedupe vs Hourly Recon (GC-RECON-*) + audit findings + BOTH '${UIC}/findings-desktop.jsonl' and '${UIC}/findings-mobile.jsonl' before minting GC-UIC-NNNN; append one line to the PER-SURFACE file '${UIC}/findings-${surface}.jsonl' (the two surfaces write separate files so simultaneous runs never race one file; status.py merges both into FINDINGS.md). Report only (no fix backlog).

STOP at budget (U7) — close out cleanly mid-inventory; leaving a case in-flight is fine (the ledger resumes it). Return the structured verdict for THIS page (page, buildStart, backend, controlOk, cases[]).`
...
const end = await agent(
  `Open ${url} via ${surface === 'mobile' ? 'chrome-devtools' : 'Playwright'} MCP (its own browser instance; no isolation needed), login admin/admin if needed, read /version.json shortCommit in-browser and return ONLY that string.`,
  { label: 'fingerprint:end' },
)
const closeout = await parallel(
  runPages.map((p) => () => agent(
    `In repo ${REPO} (WRITE ONLY under '${UIC}/**'): RUN_TS = the newest 'run' value in '${isoDir(p)}/ledger.jsonl' (the run just executed). Render page '${p}' surface ${surface} artifacts FROM its ledger (never memory, U8): (1) 'python "${UIC}/tools/status.py" --surface ${surface} --page ${p}' to rewrite STATUS.md; (2) write '${isoDir(p)}/runs/<RUN_TS>.md' with run facts (browser tool ${surface === 'mobile' ? 'chrome-devtools' : 'Playwright'}; start/end fingerprint ${JSON.stringify(results.find((r) => r.page === p)?.buildStart)} vs end '${end}' — MISMATCH invalidates the run; backend; control result; per-case table with evidence; any GC-UIC findings in full); (3) update '${isoDir(p)}/PASSDOWN.md' — prepend a fresh LATEST blockquote (coverage tally, in-flight, next-3, blockers-worth-retrying, environment notes, ${surface === 'mobile' ? 'PAGE-COMPLETE if the inventory is now complete, ' : ''}and any SKEPTIC-DISPUTED cases from: ${JSON.stringify(disputes.filter((d) => true))}) and push the previous LATEST into History. Demote any disputed case to SUSPECTED_ISSUE for next run. Return a 3-line summary.`,
