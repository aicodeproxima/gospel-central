...
WORK: run '${p}' cases in CASES.md order, RESUMING from the ledger. Determine the resume point by reading '${isoDir(p)}/ledger.jsonl' (a case with step lines but no 'case' line is in-flight -> continue at its first missing step; else the first case with no 'case' line; when none remain, do a REGRESSION lap on cases whose last build != buildStart). Budget: up to ${maxCases} case-slots (any authored case — workflow/metric-proof/chain/round-trip/failure/theme/boundary/permission-boundary = 2; atomic = 1). Priority: in-flight -> proofs/workflows/chains/round-trips/adversarial-edge -> atomics. For failure/theme/boundary/permission-boundary cases (and any negative assertion), the evidence MUST capture the ADVERSE condition actually occurring (the 403/conflict/error text, the applied theme, the overflow render, the empty state, the second submit) — a negative case whose condition never fired is BLOCKED_BY_UI_PREREQUISITE, never PASS (GUARDS "Evidence bars"). Theme cases change theme via the UI picker only (never inject the two localStorage keys); a silent cross-scope write in a permission-boundary case is a CONFIRMED_ISSUE.
