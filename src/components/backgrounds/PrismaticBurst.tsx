...
/**
 * Accepts any CSS mix-blend-mode value. `'none'` is treated as a sentinel
 * meaning "do not apply any blend mode" (the canvas style is cleared).
 */
export type PrismaticBurstMixBlendMode = NonNullable<React.CSSProperties['mixBlendMode']> | 'none';

export interface PrismaticBurstProps extends React.HTMLAttributes<HTMLDivElement> {
  intensity?: number;
  speed?: number;
  ...
  rayCount?: number;
  mixBlendMode?: PrismaticBurstMixBlendMode;
}
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // optional-method view keeps the runtime behavior identical without any.
      const asRemovable = (obj: object | null): { remove?: () => void } | null => obj;
      try {
        asRemovable(meshRef.current)?.remove?.();
      } catch {
  ...
  useEffect(() => {
    ...
    const animTypeMap: Record<PrismaticBurstAnimationType, number> = {
      rotate: 0,
      rotate3d: 1,
      hover: 2
    };
  ...
  return <div ref={containerRef} style={{ width: '100%', height: '100%', ...style }} {...rest} />;
};
