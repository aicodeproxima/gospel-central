...
## Loop 16 — Metrics-tab search-center + cross-surface defect-class sweep (2026-06-13)
User report: searching a name on the Groups **Metrics** tab does not snap-center the person (works on the 3D tree + list). Also asked WHY ~15 prior audits missed something this trivial.

- **Root cause:** the shared tree search bar's "center on this person" was wired only to two surfaces — tree-3d (camera rig consumes `externalFocusId`) and tree-list (page effect 138-146, gated `viewMode==='list'`, `scrollIntoView` on `[data-node-id]`). `TeacherMetricsCards` was a pure display component: no `highlightId` prop, no per-card ref/anchor, no scroll effect. So on Metrics, search set `externalFocusId` and nothing consumed it. Never wired — not a regression.
- **Fix (`10600ca`):** `TeacherMetricsCards` takes `highlightId`; on change it `scrollIntoView({block:'center'})` the matching `[data-metric-id]` card + pulses a `ring-2 ring-primary` for ~2.2s (mirrors the list view's ring). Non-teacher (no metrics card) = clean no-op. Page passes `highlightId={externalFocusId}`.
- **VERIFIED (deployed `diamond-7kjc0e3fc`, Chromium @412×915, login admin/admin):** 129 metric cards single-col (~31k px tall). Searched a MID-list teacher (Persis, u-mem-65) → card center-Y 514 vs viewport-center 458 (Δ56, centered ✅), in-viewport, ring applied; screenshot eyeballed (card dead-center with primary ring). First test target (Phoebe, last card) appeared off-center — that was a test artifact: the scroll container was already maxed (scrollTop 31093 = scrollH−clientH), the last card physically can't center. tsc clean, vitest 265/265.
- **Why the audits missed it (honest retro):** all ~15 loops optimized for LAYOUT FIT (fits @275, ≥44px targets, no h-scroll, doc-height) via screenshot CRAWLS — static rendered-page properties. No audit ever *exercised* the search interaction per surface. It's a cross-feature (search × active-tab) wiring gap where each component is individually correct, so component-level review can't see it. The 265 tests are all unit (layout/camera/permissions/mock) — zero integration test drives the search box. Even Loop 15's camera-lock bug was user-reported, not audit-caught: behavior bugs only ever surfaced via real use.
- **Cross-surface defect-class sweep (15-agent adversarial workflow, `wf_758201f5-d4e`):** mapped every shared toolbar/state feature × 4 surfaces (tree-3d / tree-list / metrics / pipeline) and adversarially verified each "absent" cell. **0 additional confirmed defects** beyond the metrics fix. Pipeline correctly excluded (search=users, pipeline=contacts → no target). Tree-only controls (expand-all/collapse/reset/per-node filters/jump-expandedIds) no-op on metrics/pipeline by design (they govern tree state you return to). 3D focused card has no ring (camera centering IS the highlight) — by design.
- **Optional polish (NOT done, judgment call):** tree-only toolbar buttons (Expand All / Collapse / Reset / filters) stay visible+clickable on Metrics/Pipeline where they silently no-op — could hide/disable on non-tree tabs. Deferred: they affect the tree you'll see on return.
- **Process fix going forward:** a feature × surface interaction matrix for shared controls + a smoke-level integration test that drives the search bar against each tab (the dimension no audit had).

---

## Loop 17 — Org flexibility (VA Beach scenario): live tree + location on people (2026-06-13)
User: roles change, and a branch leader + members + contacts may relocate to a new location (VA Beach) and take new roles — the app must adapt to ANY change, and must know where people are. Plan: docs/superpowers/plans/2026-06-13-org-flexibility.md (approved scope: live tree + location-on-people + subtree-move + branch-removal cascade).

- **Root cause:** `GET /groups/tree` returned a frozen `scenarioOrgTree` snapshot; role/reassignment edits mutated `usersState` (Users tab + audit reflected them) but never rebuilt the tree → the Groups view never restructured, so the app *looked* unable to adapt. People also had no location field (location existed only for room bookings).
- **Phase A (`d01f41c`) — keystone:** new `buildOrgTree(users, metrics, areas)` util (lib/utils/org-tree.ts) derives the Branch→Group→Team→Member tree LIVE from each user's `parentId`; `/groups/tree` now builds from `usersState` (matches a real backend). Added `User.locationId` (→ an Area id, orthogonal to parentId); seeded every person's home location from their branch's area; tree node `groupName` surfaces each person's location so "who's at <area>" is visible. `PUT /users/:id` accepts `locationId` (flows through generic update) + emits a relocation audit row. Builder: roots = parentless/orphaned users (a deactivated branch's subtree still renders), children sorted role-desc, metrics roll up without double-count. 7 unit tests.
- **VERIFIED (deployed `diamond-5l26vhxxs`, in-page MSW via Playwright, admin/admin):** mutated member "Andronicus" via the app's own fetch and re-fetched `/groups/tree` each time — **Relocate→VA Beach: PUT 200, tree loc `Norfolk Zion`→`Virginia Beach Zion` ✅; Reassign→team `Luke`: PUT 200, now under Luke ✅; Promote→team_leader: PUT 200, role flips ✅.** The tree adapts live to all three churn types. (Seeded areas already include `area-virginia-beach` = "Virginia Beach Zion".)
- **Phase B (partial) — relocation UI:** EditUserDialog gains a **Location** picker (self-fetches active areas; included in the update payload + a "Relocating X to Y" hint). `UpdateUserPayload.locationId` added. tsc clean, 271/271.
- **Remaining:** B surfacing (location badge/filter in Users tab + tree), C (move person/subtree flow + GroupsTab branch deactivate-with-cascade closing the orphan gap), D (full deployed VA-Beach end-to-end run).

## Loop 17 addendum — Phase A AUDIT + remediation (per-phase rule, 2026-06-13)
Per the new standing rule (audit→remediate→tune→re-verify after EACH phase), ran a 22-agent adversarial audit of Phase A (wf_484b61b8-2a7) BEFORE proceeding. **16 confirmed findings** — including issues tests + live happy-path verification never caught. Remediated in `9ee6a07`:
- **CRITICAL — server-side reassignment hole:** PUT /users/:id never called `canReassignUserToGroup` (helper existed, zero call sites). A direct API call let any leader re-graft a cross-branch user under any parent (UI scoped it; server didn't). Now enforced server-side + self-parent + descendant-cycle rejection.
- **MAJOR — people vanish on a cycle:** corrected my own hypothesis — a parentId cycle doesn't infinite-recurse, it makes those users UNREACHABLE from a root so they (and their metrics) silently disappear. Fixed with a visited-guard + forced-root pass in buildOrgTree (no-vanish guarantee) + write-boundary cycle rejection.
- **MAJOR — relocation had no permission check:** locationId accepted from anyone who could edit the user. Now gated (admin-tier OR own subtree) + target-area existence/active validation; EditUserDialog disables the field to match.
- **MAJOR — forgeable audit actor:** rows used client `body.actorId`; now stamped from the JWT viewer.
- **MINOR/TUNING:** duplicate-id double-build/double-count (build adjacency from byId.values() + id tiebreak); relocation audit row entityType→'area' (filterable); deleted the dead static org-tree island (had already diverged) + dead branchLeaderFor; seed walk-up cycle guard; O(1) area lookup. +5 buildOrgTree tests.
- **VERIFIED (deployed `diamond-1i1tj5hc2`, in-page API, admin + forged team-leader tokens):** cycle→403, self-parent→403, **non-admin cross-scope reassign→403**, cross-scope relocate→403, unknown area→400, admin relocate→200 (tree reflects), 132 nodes render (no vanish). tsc clean, 274/274, 0 lint errors.
- **Process note:** this is the per-phase-audit rule's first application — it caught a CRITICAL security hole that "tests pass + live happy-path works" would have shipped. Phase B (location picker) still needs its own audit before B is considered done.

## Loop 17 — Phase B COMPLETE: relocation UI + surfacing, audited (2026-06-13/14)
- **Finish (`3b57131` picker, `8a3342d` surfacing):** EditUserDialog Location picker (self-fetches active areas, payload); UsersTab location FILTER (Select mirroring tag pattern, explicit SelectValue) + location BADGE (MapPin under name in table, in role/status row on cards). Areas fetched alongside users; no new table columns. Subagent-implemented, diff-reviewed.
- **Catch during review:** diamond-live CLAUDE.md had 135 lines of uncommitted pre-existing project-rules content (HEAD = `@AGENTS.md` only) — NOT mine, left untouched, staged only the Phase B file. Flagged to user.
- **Phase B audit (`wf_84564180-27a`, resumed after a usage-limit cutoff) — 6 confirmed, remediated in `94605b8`:**
  - MAJOR: selecting "— no home location —"/"— no parent —" silently no-op'd with a success toast — `value||undefined` coalesced '' to undefined, JSON.stringify dropped the key, server never cleared. Now sends parent/location explicitly (incl. '' to CLEAR) only when authorized AND changed.
  - MAJOR: "Reports to" wasn't scope-gated like Location → non-admin got a server 403 the UI never warned of. Both fields now share one `canMoveTarget` gate matching the server.
  - MINOR: set location rendered blank if getAreas slow/failed → "Current location" fallback option.
  - MINOR: location filter whose area was deactivated showed "All locations" but filtered to zero → derived `effectiveLocationFilter` that self-heals to 'all' (NO setState-in-effect, so zero new lint debt — converted from the audit's suggested effect).
  - MINOR: users at a deactivated area lost their badge → resolve names from full area set (getAreasFull incl. inactive) while picker stays active-only.
- **VERIFIED (deployed `diamond-ovzv4yeu6`, in-page API + screenshot):** set location→VA Beach (200, tree reflects); **clear via locationId:'' → groupName now undefined = CLEARED** (was the silent no-op); location badges render per-user (Joseph→Newport News Zion … Simeon→Virginia Beach Zion); "All locations" filter present; dev/overseer show no location. tsc clean, 274/274, no new lint errors (2 pre-existing set-state-in-effect + 1 pre-existing unused-var remain).
- **Remaining: Phase C** — "move person/subtree to another leader" flow + GroupsTab branch deactivate-with-cascade (closes the orphan gap); then D (full VA-Beach end-to-end). Each gets its own audit per the standing rule.

## Loop 17 — Phase C step 1: cascade deactivate/restore (2026-06-14)
- **`d99ce04`:** `POST /users/:id/deactivate` + `/restore` accept `cascade`. When true, a new cycle-safe `subtreeUserRecords(rootId)` walks the node + all descendants and flips each, so an entire branch can be removed without orphaning its members (closes the orphan-gap backend). Default keeps single-user behavior; buildOrgTree's forced-root pass means orphans never vanish either way. Actor stamped from JWT viewer. `usersApi.deactivate/restore` gain an optional `cascade` arg.
- **VERIFIED (deployed `diamond-ro4y6nuhy`, in-page API):** cascade-deactivate branch leader u-branch-3 → 200, deactivatedCount 25, tree 132→107 (whole subtree gone, no orphans); cascade-restore → 200, restoredCount 25, tree 107→132 (all back). tsc clean, 274/274.
- **Phase C REMAINING (next focused push):** UI confirm-with-cascade — when deactivating a user who has active descendants, the confirm dialog must show "X has N people under them" and offer "deactivate the whole subtree" vs "cancel & reassign first" (UsersTab; optionally add deactivate to GroupsTab). Then the Phase C ADVERSARIAL AUDIT + remediation + verify, per the standing rule (Phase C is NOT done until audited).

## Loop 17 — Phase C COMPLETE: cascade deactivate + confirm UI, audited (2026-06-14)
- **Step 1 `d99ce04`** (server cascade), **Step 2 `ac468cf`** (confirm-with-cascade UI: ConfirmDialog children slot; UsersTab affectedDescendantCount + cascade checkbox defaulting ON, "Also deactivate the N people reporting under them" with the orphan warning; usersApi cascade arg).
- **Phase C audit (`wf_94424656-773`) — 9 confirmed, remediated `6fc43bb`:**
  - **CRITICAL privilege-escalation:** cascade authorized only the ROOT then flipped every descendant unchecked. Tree is parentId-derived + mutable, so a higher-ranked user (Dev/Overseer) can sit inside a lower subtree → a branch leader could lock out / re-enable someone above their authority across a whole branch, voiding "cannot modify above your own level". FIX: both loops pre-validate EVERY target with canDeactivateUser and 403 the whole request (all-or-nothing) if any outranks the viewer.
  - **MAJOR restore-resurrection:** cascade-restore revived members deactivated INDEPENDENTLY before the leader. FIX: deactivate-cascade stamps `User.deactivatedCascadeId` (batch id); restore-cascade revives only the root's batch.
  - **MINOR ×6 count drift:** deactivatedCount/restoredCount returned subtree size not actual flips (toast lied) → count real flips; the dialog descendant count is now gated by canDeactivateUser and scoped to the restore batch, so the prompt matches the server.
- **VERIFIED (deployed `diamond-6h9m6bdlk`, in-page API w/ forged branch-leader token):** **branch leader cascade-deactivating a group with a grafted Dev → 403** "above your authority" (escalation closed); admin cascade clean branch → 200, deactivatedCount 35 == tree delta 35 (exact); restore → 200, tree fully back; **independently-deactivated member NOT resurrected by a branch restore**. tsc clean, 274/274, no new lint.
- **Caveat:** the confirm-dialog CHECKBOX itself wasn't click-verified (admin Row-actions menu won't open under Playwright — pre-existing base-ui+automation quirk); server cascade + auth + count + batch all API-verified, UI wiring type-checked.
- **Remaining: Phase D** — full VA-Beach end-to-end demo run (create location → relocate a branch leader + downline with new roles → cleanly retire the old branch), which exercises the now-complete A+B+C capabilities together.

## Loop 17 — Phase D + ORG-FLEXIBILITY FEATURE COMPLETE (2026-06-14)
- **Phase D — end-to-end VA-Beach scenario (deployed `diamond-6h9m6bdlk`, in-page API, fresh state):** relocated Newport News branch leader u-branch-1 + their entire 35-person subtree to VA Beach (35/35 PUT 200); tree groupName for the branch now "Virginia Beach Zion"; promoted a member to team_leader (role reflected in tree); VA Beach headcount 17→52 (gain == subtree 35). The user's exact scenario — a branch leader + downline relocate and take new roles — works live, permission-gated.
- **FEATURE COMPLETE (plan `docs/superpowers/plans/2026-06-13-org-flexibility.md`):** A (live tree + location model) · B (relocation UI + filter/badge) · C (cascade deactivate + confirm UI) · D (end-to-end). EVERY phase implemented → adversarially audited → remediated → re-verified per the standing rule. Audits caught what tests/demos missed: a **critical server-side reassignment permission hole** (A), a **major silent clear-location no-op** (B), and a **critical cascade privilege-escalation** (C) — none of which a "tests pass + happy-path demo" would have surfaced.
- **Net commits:** A `d01f41c`+`9ee6a07`(audit) · B `3b57131`/`8a3342d`+`94605b8`(audit) · C `d99ce04`/`ac468cf`+`6fc43bb`(audit) · D verification only. tsc clean + 274/274 throughout; zero net-new lint debt.
- **Standing item:** branch `feat/mobile-opt-main` still 0 behind main (fast-forwardable); iPhone login closed; integration to main remains the user's call. The org-flexibility feature is mock-backed — the §7-shim permission contract is what Mike's real Go backend must mirror at cutover.

## Loop 18 — CRITICAL pre-existing bug found by the new adversarial gate: every admin ⋯ menu crashed (2026-06-14)
Applying the new GATE 2 (adversarially exercise the INTERACTION, not just layout) to close the "two UI dialogs not click-verified" caveat immediately surfaced a real, widespread, pre-existing defect.
- **Root cause (`84592df`):** the shared `DropdownMenuLabel` (`src/components/ui/dropdown-menu.tsx`) wrapped base-ui `<Menu.GroupLabel>`, which REQUIRES a surrounding `<Menu.Group>` context. Every call site (13 menus across Users/Groups/Rooms/BlockedSlots) uses a STANDALONE label with no group → `GroupLabel` threw "MenuGroupRootContext is missing" = **Base UI error #31** the instant any menu opened. The error boundary caught it; the menu never rendered. So Edit / Deactivate / Reset / Manage-tags / Rename — and the relocation + cascade dialogs behind them — were ALL unreachable.
- **Why it shipped:** prior admin verification (Loop 14) was layout-only (h-scroll, touch targets, fit) — it never OPENED a menu. Confirmatory verification missed a 100%-reproducible interaction crash. Exactly the failure GATE 2 / Level 0 now blocks.
- **Fix:** render a presentational `<div>` heading instead of `<Menu.GroupLabel>` in the shared wrapper — one change fixes all 13 menus; a standalone heading needs no group context. tsc clean, 274/274.
- **VERIFIED (deployed `diamond-iv9f0ms0w`, Chrome 149 — modern, not an env quirk):** Row-actions menu opens with items, **error #31 gone, zero console errors**. And the two previously-uncloseable caveat dialogs are now CLICK-verified: **EditUserDialog location picker** renders (6 options) + parent picker; **branch-leader Deactivate → cascade ConfirmDialog** shows the checkbox (default-checked), "Also deactivate the 34 people reporting under them," and the orphan warning (screenshot eyeballed).
- **Caveat #1 (UI dialogs not click-verified) is now CLOSED.** Remaining: caveat #2 (mock vs Mike's real Go backend) — at cutover only.
